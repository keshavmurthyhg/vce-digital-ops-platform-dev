...
# clean names
def clean(x):
    return str(x).split("<")[0].strip() if x else ""
...
for col in ["Created Date","Resolved Date"]:
    if col in page_df:
        page_df[col] = pd.to_datetime(page_df[col], errors="coerce").dt.strftime("%d-%b-%y").fillna("")
...
page_df["Link"] = page_df["Number"].apply(make_link)
